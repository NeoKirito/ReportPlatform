...
# Always generate from the tracked defaults, never from a previous deployment or
# private runtime directory. The only user-edited configuration is config.ini.
...
$config.ReportDatabase.ConnectionString = 'Server=192.168.0.237;Database=TJXT0616;User ID=sa;Password=Sxyckj#123;TrustServerCertificate=True;'
...
(Get-Content -LiteralPath (Join-Path $packageRoot 'config.ini') -Raw).Replace('ConnectionString=', 'ConnectionString=Server=192.168.0.237;Database=TJXT0616;User ID=sa;Password=Sxyckj#123;TrustServerCertificate=True;') | Set-Content -LiteralPath (Join-Path $packageRoot 'config.ini') -Encoding UTF8
